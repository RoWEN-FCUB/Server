...
class UsersController{
   ...
   public async list (req: Request,res: Response): Promise<void>{
    ...
    public async getSubordinados(req: Request,res: Response): Promise<void>{ 
        const {id} = req.params;
        const user = await pool.query('SELECT users.id, users.user, users.fullname FROM users WHERE id_sup = ?', [id], function(error: any, results: any, fields: any){
            if(results.length > 0) {
                res.json(results);                
            }
            else {
    ...
    public async validate(req: Request, res: Response): Promise<void>{        
        ...
        const resp = await pool.query('SELECT users.id, users.email, users.picture, users.pass, users.user, users.fullname, users_roles.role FROM users INNER JOIN users_roles ON (users.role = users_roles.id) WHERE email = ?',[email], function(error: any, results: any, fields: any){
