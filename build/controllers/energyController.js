...
class EnergyController {
    ...
    update(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            req.body.plan = Number(req.body.plan);
            req.body.lectura = Number(req.body.lectura);
            const query = 'UPDATE energia SET plan = ' + req.body.plan + ', consumo = ' + req.body.consumo + ', lectura = ' + req.body.lectura + ', plan_hpic = ' + req.body.plan_hpic + ', real_hpic = ' + req.body.real_hpic + ' WHERE id = ' + id + ';';
            // console.log(query);
            yield database_1.default.query(query, function (error, results, fields) {
    ...
    updateAll(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield database_1.default.query('INSERT INTO energia (id, plan, consumo, lectura, plan_hpic, real_hpic) VALUES ? ON DUPLICATE KEY UPDATE plan=VALUES(plan),consumo=VALUES(consumo),lectura=VALUES(lectura),plan_hpic=VALUES(plan_hpic),real_hpic=VALUES(real_hpic);', [updates], function (error, results, fields) {
                if (error) {
                    console.log(error);
                }
                res.json({ message: 'Energy record updated' });
    ...
    updatePlans(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            while (moment(inicio).isSameOrBefore(fin, 'day')) {
                erecords.push([inicio, Number(plan), Number(plan_pico)]);
                inicio = moment(inicio).add(1, 'days').format('YYYY-MM-DD');
            }
            //console.log(erecords);
            yield database_1.default.query('INSERT INTO energia (fecha, plan, plan_hpic) VALUES ? ON DUPLICATE KEY UPDATE plan=VALUES(plan), plan_hpic=VALUES(plan_hpic);', [erecords], function (error, results, fields) {
                if (error) {
                    console.log(error);
                }
                res.json({ message: 'Energy record updated' });
    ...
    deleteERecord(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            yield database_1.default.query('UPDATE energia SET consumo = null, lectura = null, plan_hpic = null, real_hpic = null WHERE id = ?', [id], function (error, results, fields) {
